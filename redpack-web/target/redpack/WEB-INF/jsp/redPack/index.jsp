...
<article>
  <div>
    <img src="${fileServerUrl }/res/img/banner.jpg" alt="">
  ...
  <div class="p1" style="margin-top:60px;">
    <a class="apply" id="modify" href="##">修改资料</a>
     <dd>${message }</dd>
  </div>
  <div class="p1" style="margin-top:30px;">
    <a class="apply" id="upgrade" href="##">我要升级</a>
     <dd>${message }</dd>
  </div>
</article>
       <!--  <a href="<c:url value='/app/elend/personalCenter'/>" class="person_center"><img src="${fileServerUrl }/app_res/img/elend/personLink.png" class="boximg" /></a> -->
        ...
        <div class="opacity" style="display:none;">
           <div class="alertcontairn">
             <span class="closeOpacity closeCcl"><img src="${fileServerUrl }/res/img/alertClose.png" class="boximg" /></span>
             <div class="textinfo">
               <div class="justword">
                 <p>注册用户可申请贷款，</p>
		...
		$('#modify').click(function(){
			...
			if(isregister){
			  window.location.href="<c:url value='/redPack/modifyInfo.do'/>"//如果已经注册跳转到贷款页面
			}
			else{
		...
		$('#upgrade').click(function(){
			var isregister=true;//自定义的虚拟变量false为用户还没注册，true用户已经注册过
			if(isregister){
			  window.location.href="<c:url value='/redPack/upgradeInfo.do'/>"//如果已经注册跳转到贷款页面
			}
			else{
			  $('.opacity').show();//没有注册弹出弹框提示用户去注册	
			}
